...
export default function Home() {
  ...
  const content = value.codes.map((code) => (
    <tr key={code.id} id={code.id % 2 === 1 ? "odd" : "even"}>
      <td>{code.id + 1}</td>
      <td>{code.nama}</td>
      <td>{code.tipe}</td>
      <td>{code.rasa}</td>
      ...
      <td>{code.keju}</td>
      <td>{code.garam}</td>
      <td className="deskripsi">{code.deskripsi}</td>
    </tr>
  ));
  ...
  return (
    // <p>Hello</p>
    <>
      <div className="font-asap mt-4 mx-4">
        <div className="w-full mx-auto text-center bg-slate-400 rounded">
          <div className="py-4">
            <p className="text-6xl font-bold">ayaa-cookies</p>
            <p className="mt-4 text-lg">Temukan kue-kue sederhana yang Ayaa Cookies buat di sini.</p>
          </div>
